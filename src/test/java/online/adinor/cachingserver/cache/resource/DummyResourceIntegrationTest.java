...
public class DummyResourceIntegrationTest {
  ...
  public static final DropwizardAppRule<MainConfiguration> DROPWIZARD
      = new DropwizardAppRule<MainConfiguration>(MainApplication.class, ResourceHelpers.resourceFilePath("test-config.yml"));
  ...
  public void testGetBare() throws Exception {
    ...
    Response response = client.target(String.format("http://localhost:%d/bare", DROPWIZARD.getLocalPort()))
        .request()
        .get();
  ...
  public void allParallelRequestsProduceIdenticalResponses() throws Exception {
    final int NUMBER_OF_PARALLEL_REQ = 20;
    final String path = String.format("http://localhost:%d/cached", DROPWIZARD.getLocalPort());
    ...
    class ClientRunner implements Runnable {
      ...
      public void run() {
        synchronized (threadsNo) {
          ...
          try {
            threadsNo.wait();
            response.set(client.target(path)
                .request()
                .get());
    ...
    JerseyClient client = new JerseyClientBuilder().build();
    Response response1 = client.target(path)
        .request()
        .get();
    synchronized (threadsNo) {
      threadsNo.notifyAll();
    ...
    Assertions.assertThat(response1.getStatus()).isEqualTo(200);
    int i = 0;
    for (AtomicReference<Response> ar : responses) {
      System.out.println("Step: " + (++i));
      Assertions.assertThat(ar.get().readEntity(String.class)).isEqualTo(firstEntity);
    }
  }
  ...
  public void twoResponseDiffersAfter10seconds() throws Exception {
    ...
    final Response response1 = client.target(String.format("http://localhost:%d/cached", DROPWIZARD.getLocalPort()))
        .request()
        .get();

    Thread.sleep(10_001);

    final Response response2 = client.target(String.format("http://localhost:%d/cached", DROPWIZARD.getLocalPort()))
        .request()
        .get();
    ...
    Assertions.assertThat(entity1).isNotEqualToIgnoringCase(entity2);
  }
